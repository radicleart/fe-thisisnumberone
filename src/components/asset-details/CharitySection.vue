...
<section v-if="content" class="container-fluid p-5 hundred-vh bg-white text-black">
  <b-container>
    <b-row>
      <div v-if="content.image" id="charity-column" class="col-md-6 col-sm-12">
        <img :src="content.image.url" width="100%"/>
      </div>
      <div class="col-md-6 col-sm-12">
        <b-row class="p-0 m-0" align-v="stretch" :style="'height:' + videoHeight + 'px;'">
...
export default {
  ...
  computed: {
    content () {
      const item = this.$store.getters['myItemStore/myItem'](this.assetHash)
      if (!item) return
      let prismicId = 'martlets'
      if (item.prismicId) prismicId = item.prismicId
      const content = this.$store.getters[APP_CONSTANTS.KEY_CONTENT_CHARITY_BY_ID](prismicId)
