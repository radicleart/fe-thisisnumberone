...
<div class="text-small pb-0 mb-0">
  <span v-if="this.item.contractAsset">
    ...
    <span class="ml-2"><a class="text-warning" :href="application.tokenContract.baseTokenUri + item.contractAsset.nftIndex" v-b-tooltip.hover="{ variant: 'light' }" :title="'Base Token URL: ' + application.tokenContract.baseTokenUri + item.contractAsset.nftIndex" target="_blank">nft link</a></span>
    <span class="ml-2" v-if="item.mintInfo"><a :href="transactionUrl" v-b-tooltip.hover="{ variant: 'light' }" :title="'Base Token URL: ' + application.tokenContract.baseTokenUri + item.contractAsset.nftIndex" target="_blank">mint tx</a></span>
  </span>
  <span v-else>
